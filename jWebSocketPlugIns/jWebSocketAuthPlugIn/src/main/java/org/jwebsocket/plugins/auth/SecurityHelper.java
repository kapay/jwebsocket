...
public class SecurityHelper {
	...
	public static Boolean isUserAuthenticated(String aSessionId) {
		try {
			...
			if (session.containsKey(AuthenticationPlugIn.IS_AUTHENTICATED)) {
				return (Boolean) session.get(AuthenticationPlugIn.IS_AUTHENTICATED);
			}
			return false;
		} catch (Exception ex) {
	...
	public static boolean userHasAuthority(String aSessionId, String aAuthority) {
		try {
			Map<String, Object> session = getSessionManager().getSession(aSessionId);
			if (session.containsKey(AuthenticationPlugIn.AUTHORITIES)) {
