...
public final class JWebSocketXmlConfigInitializer implements WebSocketInitializer {
  ...
  public void initializeLogging() {
    ...
    Logging.initLogs(lLoggingConfig.getLevel(), lLoggingConfig.getAppender(), lLoggingConfig.getFilename(), lLoggingConfig.getPattern(),
        lLoggingConfig.getBufferSize(), new String[] {
        "%JWEBSOCKET_HOME%/logs", "%CATALINA_HOME%/logs" });
    mLog = Logging.getLogger(JWebSocketXmlConfigInitializer.class);
    if (mLog.isDebugEnabled()) {
      mLog.debug("Logging settings" + ": appender: " + lLoggingConfig.getAppender() + ", filename: " + lLoggingConfig.getFilename() 
          + ", level: " + lLoggingConfig.getLevel() + ", buffersize: "
  ...
  public WebSocketEngine initializeEngine() {
    ...
    String jarFilePath = "-";
    try {
      Class<WebSocketEngine> lEngineClass = null;
      // try to load engine from classpath first,could be located in server bundle
      try {
        lEngineClass = (Class<WebSocketEngine>)Class.forName(engineConfig.getName());
        if (mLog.isDebugEnabled()) {
      ...
      // if not in classpath...try to load engine from given .jar file
  ...
  public List<WebSocketServer> initializeServers() {
    ...
    for (ServerConfig lServerConfig : lServerConfigs) {
      ...
      String lJarFilePath = "-";
      try {
        Class<WebSocketServer> lServerClass = null;

        // try to load server from classpath first, could be located in server bundle
        try {
          lServerClass = (Class<WebSocketServer>)Class.forName(lServerConfig.getName());
          if (mLog.isDebugEnabled()) {
  ...
  public Map<String, List<WebSocketPlugIn>> initializePlugins() {
    ...
    for (PluginConfig lPluginConfig : mConfig.getPlugins()) {
      try {
        ...
        // try to load plug-in from classpath first, could be located in server
        // bundle
        try {
